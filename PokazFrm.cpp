...
void PokazFrm::CreateGUIControls() {
	...
	BGPanel->Bind(wxEVT_LEFT_DCLICK, &PokazFrm::FullScreen, this);
	this->Show();
...
void PokazFrm::drawBackground() {
    ...
    BGPanel->GetSize(&w, &h);
    dc->DrawBitmap( wxBitmap(background.Scale(w,h)), 0, 0, true );
...
void PokazFrm::drawImages() {
    ...
    for (std::vector<SlideImage>::iterator it = images.begin(); it != images.end(); ++it,  x += 250) {
        for(int i = 0;  i <= 600 ; i += 2) {
            dc->DrawBitmap( wxBitmap(background.Scale(w,h)), 0, 0, true );
            dc->DrawBitmap( wxBitmap( (*it).Scale(200,200)), 50+i, 250, true );
            client->Blit(0, 0, w, h, dc, 0, 0, wxCOPY );
            //Sleep(5);
            wxMilliSleep(5);
        }
    }
